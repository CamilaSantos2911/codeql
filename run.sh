...
check_sq_is_up() {
  local statusCall="$(curl --silent --user admin:carboidrato2911 http://127.0.0.1:9000/api/system/status)"
...
docker run -v `pwd`:/github/workspace/ --workdir /github/workspace --network $network --env SONAR_TOKEN=$token --env SONAR_HOST_URL='http://localhost:9000' sonarsource/sonarqube-scan-action
if [[ $? -eq 0 ]]; then
  error "Should have failed fast."
  exit 1
fi
